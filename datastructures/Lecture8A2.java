    ...
    public static void main(String args[]) throws IOException
    {
        ...
        {  
            ...
            for (int j = 0; j < samples; j++)
            {
                ...
                System.arraycopy(list, 0, list3, 0, listsize);
                ...
                countingSort(list3, 0, listsize);
                time = System.currentTimeMillis() - startns;
                mean[2] += time;
                myWriter.write(String.valueOf(time));
                System.out.println("Count sort finished " + listsize + " size list in " + time + "ms");

                myWriter.write("\n");  
                System.gc();
            }

            long best = 999999999999l;
            for (int j = 0; j < 3; j++)
            {
                mean[j] /= samples;
                best = Math.min(mean[j], best);
            }
            
            System.out.println("Quick sort mean: " + mean[0] + "ms. " + (best != 0f? String.valueOf(((float)mean[0]/(float)best)*100f) + "%" : ""));
            System.out.println("Merge sort mean: " + mean[1] + "ms. " + (best != 0f? String.valueOf(((float)mean[1]/(float)best)*100f) + "%" : ""));
            System.out.println("Count sort mean: " + mean[2] + "ms. " + (best != 0f? String.valueOf(((float)mean[2]/(float)best)*100f) + "%" : ""));
        }
